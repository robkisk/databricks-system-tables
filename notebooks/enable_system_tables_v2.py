...
r = requests.get(
    f"{host}/api/2.0/unity-catalog/metastores/{metastore_id}/systemschemas",
    headers=headers,
).json()
...
for schema in schemas_to_enable:
    ...
    r = requests.put(
        f"{host}/api/2.0/unity-catalog/metastores/{metastore_id}/systemschemas/{schema}",
        headers=headers,
    )
    if r.status_code == 200:
        ...
        print(
            f"""Error enabling the schema `{schema}`: {r.json()["error_code"]} | Description: {r.json()["message"]}"""
        )
    sleep(1)
